<template>
    <article class="flex-column-justify-space-evenly-items-center profile-container" v-if="!!userInfo">
        ...
        <!-- FIXED: Use Vue Router instead of backend link -->
        <RouterLink to="/profile/edit" class="edit-link">
            Rediger min side
        </RouterLink>
        ...
        <header class="flex-column-justify-space-evenly-items-center profile-content">
            <p>
                Bruker Navn :
                <a :href="userInfo?.id ? `https://discordapp.com/users/${userInfo.id}` : '#'" target="_blank">
                    {{ userInfo?.username }}
                </a>
            </p>
            ...
            <p v-if="userInfo && 'phone' in userInfo && userInfo.phone">
                Telefon :
            ...
            <address>
                <p>
                    Fylke / Kommune:
                    {{ (userInfo as any)?.location?.county || 'Ikke lagt til' }},
                    {{ (userInfo as any)?.location?.city || 'Ikke lagt til' }}
                </p>
            </address>
        </header>
